...
import static edu.colorado.clear.liblinearbinary.Linear.closeQuietly;
import static edu.colorado.clear.liblinearbinary.Linear.printf;
        ...
        while ((line = reader.readLine()) != null) {
            ...
            while (st.hasMoreTokens()) {
                ...
                try {
                    ...
                    if (idx <= nr_feature) {
                        Integer node = Integer.valueOf(idx);
                        x.add(node);
                    }
            ...
            if (model.bias >= 0) {
                Integer node = Integer.valueOf(n);
                x.add(node);
            }

            int[] nodes = new int[x.size()];
	    for (int i=0; i<nodes.length; ++i)
		nodes[i] = x.get(i).intValue();
