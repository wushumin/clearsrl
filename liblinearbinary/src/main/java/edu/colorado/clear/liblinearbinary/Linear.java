package edu.colorado.clear.liblinearbinary;
...
public class Linear {
    ...
    public static int predict(Model model, int[] x) {
        double[] dec_values = new double[model.nr_class];
        return predictValues(model, x, dec_values);
    }

    public static int predictProbability(Model model, int[] x, double[] prob_estimates) {
    ...
    public static int predictValues(Model model, int[] x, double[] dec_values) {
        int n;
        if (model.bias >= 0)
    ...
    private static void solve_l1r_l2_svc(Problem prob_col, double[] w, double eps, double Cp, double Cn) {
        ...
        for (j = 0; j < w_size; j++) {
            ...
            index[j] = j;
            xj_sq[j] = 0;
            for (int xi : prob_col.x[j]) {
                int ind = xi - 1;
                //xi.value *= y[ind]; // x->value stores yi*xij
                xj_sq[j] += C[GETI(y, ind)];
            }
        }
    ...
    static Problem transpose(Problem prob) {
        ...
        for (int i = 0; i < n; i++) {
            prob_col.x[i] = new int[col_ptr[i + 1]];
        ...
        for (int i = 0; i < l; i++) {
            for (int j = 0; j < prob.x[i].length; j++) {
                int x = prob.x[i][j];
                int index = x - 1;
                prob_col.x[index][col_ptr[index]] = i + 1;
                col_ptr[index]++;
            }
        }
